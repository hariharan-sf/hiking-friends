...
These geolocation properties are Default Properties of Mixpanel’s Ingestion APIs and Client-side SDKs.
...
Our ingestion API receives a request to ingest an event or update a profile.
...
Django exposes this in the request object with request.META['REMOTE_ADDR'].
Flask exposes this as request.remote_addr.
Go exposes this as the RemoteAddr field on the http.Request struct.
Nginx exposes this as $remote_addr.
Events
